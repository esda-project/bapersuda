-- ============================================================
-- SIANGGARAN — Security Fix (VERSI FINAL, SUDAH BENAR)
-- Kolom benar: anggaran.nama_belanja | realisasi.nominal
-- Jalankan SATU FILE INI SAJA di Supabase SQL Editor
-- ============================================================

-- STEP 1: DROP SEMUA VIEWS
...
SELECT
  id, kode_rekening, program, kegiatan, sub_kegiatan,
  kode_sub_kegiatan, kode_belanja, nama_belanja,
  sumber_dana, pptk, tahun,
  pagu_murni, pagu_pergeseran, pagu_perubahan,
  COALESCE(NULLIF(pagu_perubahan,0), NULLIF(pagu_pergeseran,0), pagu_murni, 0) AS pagu
FROM public.anggaran;
GRANT SELECT ON public.v_pagu_aktif TO anon;

-- [2] v_realisasi_per_rekening  ← nominal, BUKAN jumlah
...
SELECT
  p.kode_rekening, p.program, p.kegiatan, p.sub_kegiatan,
  p.kode_sub_kegiatan, p.kode_belanja, p.nama_belanja,
  p.sumber_dana, p.pptk, p.tahun,
  p.pagu, p.pagu_murni, p.pagu_pergeseran, p.pagu_perubahan,
...
SELECT sub_kegiatan, kode_sub_kegiatan, tahun,
  SUM(pagu) AS total_pagu, SUM(total_realisasi) AS total_realisasi, SUM(sisa) AS total_sisa
...
SELECT
  ...
  p.kode_sub_kegiatan, p.kode_belanja, p.nama_belanja,
  p.sumber_dana, p.pptk, p.tahun,
  p.pagu_murni, p.pagu_pergeseran, p.pagu_perubahan,
